...
- `history.json` is rebuilt from dated snapshots on every run. The current month uses the latest snapshot (the same one the cards use) rather than the 1st, so the chart's final point always equals the card scores; `main()` asserts this and exits non-zero on mismatch. Earlier versions ended the chart on the 1st while cards used the latest day, which read as a bug on the page. It, so it is reproducible and self-healing. Months with no published snapshot are skipped; a model absent from a snapshot gets `null`, which the chart draws as a visible gap (`spanGaps: false`). Never backfill or interpolate these — an estimated point is indistinguishable from a real one once it is on the chart.
